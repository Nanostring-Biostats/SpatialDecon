---
title: "Use of SpatialDecon in a large GeoMx dataset"
output: 
  rmarkdown::html_vignette: 
    toc: true
vignette: >
  %\VignetteIndexEntry{Use of SpatialDecon in a large GeoMx dataset}
...
```{r loaddata}
data("nsclc")
norm = nsclc$normalized
raw = nsclc$raw
annot = nsclc$annot
geneannot = nsclc$geneannot
dim(raw)
head(annot)
...
bg = derive_GeoMx_background(norm = norm,
                             probepool = geneannot[rownames(norm), "Pooling"],
...
o = hclust(dist(t(temp[, annot$AOI.name == "TME"])))$order
TIL_barplot(restils$prop_of_nontumor[, annot$AOI.name == "TME"][, o], 
