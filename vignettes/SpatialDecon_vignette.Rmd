...
A note for background estimation: in studies with two probesets, the genes from each probeset will have distinct background values, and the above code should be run separately for each probeset using its corresponding NegProbe value. Alternatively, the "derive_GeoMx_background_at_normalized_scale" can do this automatically:

```{r derivebg}
bg2 = derive_GeoMx_background_at_normalized_scale(norm = norm,
...
The SpatialDecon library includes several helpful functions for further analysis/fine-tuning of decon results. 
...
The function "convertCellTypes" takes a decon results object and collapses any colsely-related cell types you tell it to:
...
If your data includes any regions that are purely this unmodelled cell type - e.g. becuase you've used the GeoMx platform's segmentation capability to specifically select them - then you can infer a profile for that cell type and merge it with your cell profile matrix. 
...
2. Fitted expression vlaues for each gene based on cell mixing.
...
```{r reverse, fig.height=6, fig.width=6}
